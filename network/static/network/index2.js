  ...
  fetch(`new2?page=${no}`)
    ...
    element2.style.display = 'none';
        if(usern== post.sender){
          element.innerHTML=`${post.sender} at ${post.timestamp}<br>     <span id="body${post.id}" >${post.body}</span><br><span id="a${post.id}"><br><i class="material-icons like" class="like">favorite</i> </span><span id="${post.id}">${post.like}</span><br>
